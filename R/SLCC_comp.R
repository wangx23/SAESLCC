#' this code gives estimator for comp estimator. 
#' 
#' 
...
SLCC_comp <- function(area, y, x, Xbar, 
                    wts, N, model, group=NULL, 
                    lambda,standardize = TRUE, center = TRUE)
{
  nt <- length(y)
  ...
  if(model == "intercept")
  {
    ...
    refit_comp <- estSAE_comp(obj = resm,indexy = area,y = y,
  ...
  out <- list(resm = resm, BICm = BICm,
              refit = refit_comp, cluster = resm$cluster, 
              estY =  refit_comp$estY,
              init = betam01)
  return(out)
}