...
############ below are not used in the current version #####
# refitzxwise <- function(indexy, y, z, x, groupmat)
# {
#   ncx <- ncol(x)
#   ncz = ncol(z)
#   N <- length(y)
#   nr <- nrow(groupmat)
#   ngest <- apply(groupmat, 2, function(x){length(unique(x))})
#   ngtotal <- sum(ngest)
#   
#   Xm <- matrix(0, N, nr*ncx)
#   ym <- rep(0, N)
...
#     nJ[i] <- sum(indexy == uniqxy[i])
#     Xm[indexy == uniqxy[i],(ncx*(i-1) + 1) : (ncx*i)] <- x[indexy == uniqxy[i],]
#     ym[indexy == uniqxy[i]] <- y[indexy == uniqxy[i]]
#   }
#   
#   
#   
...
#   {
#     W <- matrix(0, ncx, ngtotal)
#     W[cbind(1:ncx,groupmat[i,] + ngestcum)] <- 1
#     return(W)
#   }
#   Wmat <- do.call("rbind",lapply(1:nr, wfun))
#   
...
#   
#   sig2 <- sum((ym - Ux%*%est)^2)/N
#   
#   vcov <- sig2*solve(t(Ux)%*%Ux)
#   
#   out <- list(est = cbind(est = est,dim = c(1:ncz,rep(1:ncx, ngest))), sig2 = sig2, vcov = vcov)
#   
#   
#   return(out)
# }
...
## zx 
# refitzx <- function(group,x, z, y, indexy)
# {
#   nr <- length(group)
#   ncx <- ncol(x)
#   ncz <- ncol(z)
#   Xm <- matrix(0, nrow(x), nr*ncx)
#   zm <- matrix(0, nrow(x), ncol(z))
#   ym <- rep(0, nrow(x))
#   uniqxy <- unique(indexy)
#   nJ <- rep(0, nr)
#   # for(i in 1:nr)
...
#   #   nJ[i] <- sum(indexy == uniqxy[i])
#   #   Xm[indexy == uniqxy[i],(ncx*(i-1) + 1) : (ncx*i)] <- x[indexy == uniqxy[i],]/sqrt(nJ[i])
#   #   zm[indexy == uniqxy[i],] <- z[indexy == uniqxy[i],]/sqrt(nJ[i])
#   #   ym[indexy == uniqxy[i]] <- y[indexy == uniqxy[i]]/sqrt(nJ[i])
#   # }
#   
#   for(i in 1:nr)
...
#     nJ[i] <- sum(indexy == uniqxy[i])
#     Xm[indexy == uniqxy[i],(ncx*(i-1) + 1) : (ncx*i)] <- x[indexy == uniqxy[i],]
#     zm[indexy == uniqxy[i],] <- z[indexy == uniqxy[i],]
#     ym[indexy == uniqxy[i]] <- y[indexy == uniqxy[i]]
#   }
#   
#   
...
#   Ux <- cbind(zm,Xm%*%W)
#   est <- solve(t(Ux)%*%Ux)%*%t(Ux)%*%ym
#   sig2 <- sum((ym - Ux%*%est)^2)/length(y)
#   
#   vcov <- sig2*solve(t(Ux)%*%Ux)
#   
#   out <- list(est = est, sig2 = sig2, vcov = vcov)
#   return(out)
#   
# }